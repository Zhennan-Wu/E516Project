...
# === CONFIG ===
CONDA_ENV="elm"
INPUT_PATH="/home/exouser/shared_data"
OUTPUT_PATH="../output"
HOSTFILE_PATH="hostfile.txt"   # <-- Default hostfile for multi-node run
# ==============
...
if [ "$MULTI_NODE" -eq 1 ]; then
    # Multi-node mode
    MPI_CMD="mpiexec --hostfile $HOSTFILE_PATH -n $TOTAL_PROCS python $SCRIPT $INPUT_PATH $OUTPUT_PATH $TIMESTEPS $M $N"
else
    # Single-node mode
