...
if [ "$MULTI_NODE" -eq 1 ]; then
    ...
    MPI_CMD="mpiexec --hostfile $HOSTFILE_PATH -n $TOTAL_PROCS env \"PATH=/home/exouser/miniforge3/envs/$CONDA_ENV/bin:$PATH\" conda run -n $CONDA_ENV python $SCRIPT $INPUT_PATH $OUTPUT_PATH $TIMESTEPS $M $N --mca btl_base_verbose 100 --display-map"
else
